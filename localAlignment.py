...
class LocalAlignment:
    ...
    print(" ")
    match = int(input("Input the MATCH :" )) 
    mismatch = int(input("Input the MISMATCH : ")) 
    gap_Penalty = int(input("Input the GAP PENALTY :" )) 
    def localAlignmentScore(self): 
        ...
        while x in range(np.size(cols)): #looping over the range of the size of column 
            ...
            # count_Column = op.add(count_Column,LocalAlignment.mismatch) #performs the addition between the mismatch penalty and the count_Column
        ...
        while x in range(np.size(rows)): 
            ...
            # count_Row = op.add(count_Row,LocalAlignment.mismatch) #here we are assigning elements of the first row values with respect to the mismatch penalty localScore of the previous index 
        ...
        while row in range(1, np.size(rows)): #loops through all the iterations from 2nd row onwards in this loop as we have already assigned values to the 1st column and 1st row
            ...
            while col in range(1, np.size(cols)):
                ...
                if op.ne(sequence_1[row-1],sequence_2[col-1]):
                    if(temp_List.extend([DP_Matrix[row-1][col-1] + LocalAlignment.mismatch]) < 0):
                        temp_List.extend([0])
                    temp_List.extend([DP_Matrix[row-1][col-1] + LocalAlignment.mismatch])
                else: 
                    if(temp_List.extend([DP_Matrix[row-1][col-1] + LocalAlignment.match]) < 0):
                        temp_List.extend([0])
                    temp_List.extend([DP_Matrix[row-1][col-1] + LocalAlignment.match])
                temp_List.append(DP_Matrix[row-1][col] + LocalAlignment.gap_Penalty) #by default we compute the addition of the value to the left of the DP_matrix and gap penalty  
                temp_List.append(DP_Matrix[row][col-1] + LocalAlignment.gap_Penalty) #by default we compute the addition of the value to the above of the DP_matrix and gap penalty   
...
class PrintGrid: #this is my PrintGrid Class
    ...
    def printDPMatrix(self):
        ...
        max = 0; #this is the max localScore of this Global Alignment Matrix, which is assigned to 0 for the sake of keeping track
        print('DP Matrix') 
        for x in self.DP_Matrix: #loops through all the rows of the given DP_Matrix
            ...
            for y in x : #loops through each and every element of the row x 
                if op.gt(y ,max): #checks if the element y from the row is greater than the max localScore 
                    max = y #if the element y is greater, value y is assigned to Max 
                    continue
                else:
                    continue
        
        print("Max localScore = ", int(max)) #prints the max element of the given sequence
        print(" ") 
...
class BackTracking: #
    ...
    def backtracking(self):
        ...
        def tempList(i, j , k):
            ...
            if op.eq(op.add(self.DP_Matrix[i-one][j-one],LocalAlignment.match),(self.DP_Matrix[i][j])):
                if op.eq(self.sequence_1[i-one],self.sequence_2[j-one]):
                    tmpCheckList.extend([1])   #we add 1 to this list if the match condition is proved to be correct           
            if op.eq(op.add(self.DP_Matrix[i-1][j-one],LocalAlignment.mismatch),self.DP_Matrix[i][j]):
                if one>zero:
                    tmpCheckList.extend([one])#we add 1 to this list if the mismatch condition is proved to be correct 
            if op.eq(op.add(self.DP_Matrix[i-one][j*one],LocalAlignment.gap_Penalty),self.DP_Matrix[i][j]):
                if one>zero: 
            ...
            if op.eq(op.add(self.DP_Matrix[i][j-one],LocalAlignment.gap_Penalty),self.DP_Matrix[i][j]): 
                ...
                if(np.size(stringSet)>0):
                    ...
                    if op.gt(np.size(tmpCheckList),0): 
                        ...
                        if op.eq(tmpCheckList[0*1],1): 
                            ...
                            b = op.sub(b,1)
                            tmpCheckList.pop(0)
                        localScore(a, b, tempList(a,b,0), sequenceA, sequenceB,maxIndexSet) 
                        ...
                        if op.ne(np.size(stringSet), 0): 
                            ...
                            localScore (tempi, tempj, tempcheckarr, tempstringa, tempstringb , maxIndexSet) 
        ...
        def deleteOperations(stringSet , maxIndexSet, temporary_List):
            del (stringSet[0] , maxIndexSet[0] , temporary_List[0]) #delete the branch that's about to be followed up on
        
            
        localScore(i, j, temporary_List[0], astring, bstring , maxIndexSet)
        count = 1;
        x = 0
        while x in range(np.size(final_1st)): 
            ...
            print ('Score =' , int(self.max))  #prints the Score 
            print(" ") 
            x = op.add(x,1)
            count = op.add(count,1)
p1 = LocalAlignment('ATCAGAGTA', 'TTCAGTA')
p1.scoring()
p1.localAlignmentScore()